 ...
 */
@RestController
//@CrossOrigin(origins = "http://localhost:3000/note")
//@CrossOrigin(origins = "*")
@Api
...
public class NoteController {
	 ...
	 */
	//@CrossOrigin(origins = "*")
	@ApiOperation(value = "Create the Note")
	@PostMapping()
	public ResponseEntity<?> addNote(@RequestBody Note note) {
		if (noteService.createNote(note)) {
			return new ResponseEntity<Note>(note, HttpStatus.CREATED);
		} else {
			return new ResponseEntity<String>("Something has gone Wrong Please try again", HttpStatus.CONFLICT);
		}
	 ...
	 */
	@ApiOperation(value = "Delete note by a user")
	...
	public ResponseEntity<String> deleteNote(@PathVariable String userId, @PathVariable() int id) {
		if (noteService.deleteNote(userId, id)) {
			return new ResponseEntity<String>("Successfully deleted the u dont want Note", HttpStatus.OK);
		} else {
	...
	@ApiOperation(value = "Delete all the notes by a user")
	@DeleteMapping("/{userId}")
	public ResponseEntity<String> deleteAllNotes(@PathVariable() String userId) {
		try {
			noteService.deleteAllNotes(userId);
			return new ResponseEntity<String>("Successfully deleted the notes", HttpStatus.OK);
	 ...
	 * method.
	 */
	@ApiOperation(value = "Update The User Note")
	 ...
	 * method
	 */
	@ApiOperation(value = "Retrieve all notes by a User")
	@GetMapping("/{userId}")
	 ...
	 * 
	 */
	@ApiOperation(value = "Retrieve The Note by a User")
